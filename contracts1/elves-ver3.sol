        ...
        return
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(abi.encodePacked(id, sentinel))
                )
            );
    }
    ...
    function validSignature(
        ...
        return
            _isSignedByValidator(
                encodeForSignature(id, owner, sentinel),
                signature
            );
    }
...
*/
    ...
    function withdrawSomeTokenBalance(uint256 amount) external {
        require(bankBalances[msg.sender] > 0, "NoBalance");
        require(
            bankBalances[msg.sender] - amount >= 0,
    ...
    function _actions(
        ...
        if (action == 0) {
            ...
            (elf.primaryWeapon, elf.weaponTier) = _rollWeapon(
                elf.level,
                id_,
                rand
            );
        } else if (action == 6) {
            //item or merchant loop

            require(msg.value >= .01 ether);
            require(elf.action != 3); //Cant roll in passve mode
            (elf.weaponTier, elf.primaryWeapon, elf.inventory) = DataStructures
                .roll(
                    id_,
                    elf.level,
                    rand,
            ...
            {
                DataStructures.Elf memory hElf = DataStructures.getElf(
                    sentinels[campaign_]
                ); //using the campaign varialbe for elfId here.
                require(
                    ownerOf[campaign_] == msg.sender ||
                        hElf.owner == msg.sender,
                    "NotYourElf"
                );
                ...
                if (block.timestamp < hElf.timestamp) {
                    ...
                    actions.timeDiff = actions.timeDiff > 0
                        ? hElf.sentinelClass == 0 ? 0 : hElf.sentinelClass == 1
                            ? (actions.timeDiff * 1) / 4
                ...
                sentinels[campaign_] = DataStructures._setElf(
                    hElf.owner,
                    hElf.timestamp,
                    hElf.action,
                    hElf.healthPoints,
    ...
    function _exitPassive(uint256 timeDiff, uint256 _level)
        private
    ...
    function _rollWeapon(
        ...
        uint256 id,
        uint256 rand
    ) internal pure returns (uint256 newWeapon, uint256 newWeaponTier) {
    ...
    function attributes(uint256 _id)
        external
        view
        returns (
            uint256 hair,
        ...
        )
    {
        ...
        uint256 _traits = uint256(uint8(character >> 240));
        uint256 _class = uint256(uint8(character >> 248));
        ...
        )
    {
        ...
        timestamp = uint256(uint40(character >> 160));
        action = uint256(uint8(character >> 200));
        healthPoints = uint256(uint8(character >> 208));
        attackPoints = uint256(uint8(character >> 216));
        primaryWeapon = uint256(uint8(character >> 224));
        level = uint256(uint8(character >> 232));
    }
    ...
    function onlyOwner() internal view {
        require(
            admin == msg.sender ||
                ...
                dev1Address == msg.sender ||
                dev2Address == msg.sender
        );
    }
    ...
    function setElfManually(
        uint256 id,
        uint8 _primaryWeapon,
        uint8 _weaponTier,
        uint8 _attackPoints,
        uint8 _healthPoints,
        uint8 _level,
