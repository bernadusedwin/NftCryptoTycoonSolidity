    ...
    async execute() {
        await describe("Greeter", async function () {
            it("Should return the new greeting once it's changed", async function () {
                ...
                // {
                //     console.log("combineCargoBatch - 2");
                ...
                {
                    await expect(
                        campaign.connect(addr2).gameEngine(1, 2, [1])
                    ).to.be.revertedWith("not elf contract");
                }

                {
                    await expect(
                        trainTycoon.connect(addr2).setCargo(1, 1, 1)
                    ).to.be.revertedWith("not combine manager");
                }

                {
                    await expect(
                        trainCargoNew.connect(addr2).burnTarget(address1, 1, 1)
                    ).to.be.revertedWith("not combine manager");
                }

                {
                    await expect(
                        trainCargoNew.connect(addr2).mintTargetNoCost(address1, 1, 1)
                    ).to.be.revertedWith("not combine manager");
                }

                {
                    await expect(
                        trainCargoNew.connect(addr2).mintTargetWithPay(address1, 1, 1)
                    ).to.be.revertedWith("not train manager");
                }
