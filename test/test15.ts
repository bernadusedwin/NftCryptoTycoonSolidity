        ...
        await describe("Greeter", async function () {
            it("Should return the new greeting once it's changed", async function () {
...

                {
                    ...
                    // const options = { value: ethers.utils.parseEther("1") }
                    const trx1 = await trainCargoNew.connect(addr1).mintFragment(2, 1);
                    await trx1.wait();
                }
...

                {
                    ...
                    const trx1 = await combineManager.connect(addr1).combineCargoBatch(1, [1, 1, 2, 1]);
                    await trx1.wait();
                }
                ...
                {
                    console.log("combineCargoBatch");
                    const trx1 = await combineManager.connect(addr1).combineCargoBatch(1, [1, 2, 2, 0]);
                    await trx1.wait();
                }
                ...
                // {
                //     const trx1 = await trainTycoon.upgradeWeapon(1);
                //     console.log("inquiry cargo on train 1", trx1);
                // }
...

                {
                    ...
                    const trx1 = await trainTycoon.connect(addr1).sendCampaign([1], 2);
                    await trx1.wait();
                }

                {
                    ...
                    expect(trx1).to.equal(BigNumber.from((105 * (10 ** 18)).toString()));
                    // expect(trx1).to.equal(BigNumber.from("105" + (10 ** 18).toString()));
                }
