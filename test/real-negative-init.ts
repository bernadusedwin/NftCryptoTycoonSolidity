    ...
    async execute() {
        await describe("Greeter", async function () {
            it("Should return the new greeting once it's changed", async function () {
                ...
                {
                    await expect(
                        trainTycoon.connect(addr2).setAddresses(ercAddress, campaignAddress, trainCargoNewAddress)
                    ).to.be.revertedWith("not_admin");
                }

                {
                    await expect(
                        trainTycoon.connect(addr2).setCombineManager(trainAddress)
                    ).to.be.revertedWith("not_admin");
                }
                ...
                {
                    await expect(
                        eRC20.connect(addr2).setMinter(ercAddress, true)
                    ).to.be.revertedWith("NOT ALLOWED TO RULE");
                }

                // campaign
                {
                    await expect(
                        campaign.connect(addr2).initialize(ercAddress)
                ...
                {
                    await expect(
                        trainCargoNew.connect(addr2).initialize(1)
                    ).to.be.revertedWith("Initializable: contract is already initialized");
                }

                {
                    await expect(
                        trainCargoNew.connect(addr2).setCombineManager(trainAddress)
                    ).to.be.revertedWith("Ownable: caller is not the owner");
                }

                {
                    await expect(
                        trainCargoNew.connect(addr2).setTrainManager(trainAddress)
                    ).to.be.revertedWith("Ownable: caller is not the owner");
                }
                ...
                {
                    await expect(
                        inquiryTrain.connect(addr2).setAddresses(ercAddress)
                    ).to.be.revertedWith("not_admin");
                }
                ...
                {
                    await expect(
                        combineManager.connect(addr2).setAddresses(trainAddress, trainCargoNewAddress)
                    ).to.be.revertedWith("not_admin");
                }
